...
exports.login = async (req, res) => {
  ...
  const token = jwt.sign({ _id: user._id,role:user.user_role,email:user.email }, process.env.TOKEN_SECRET);
...
exports.resetPassword = async (req, res) => {
  ...
  try {
    if (req.body.new_password === req.body.confirm_password) {
      ...
      return res.json({data:updatedUser,msg:"Password Reset Successfully"});
    } else {
...
exports.forgetPassword = async (req, res) => {
    ...
    const Link = "<a href='http://islamic.nvinfobase.com/#/reset/?id="+ emailExist._id +"&&token=" + token + "' style='color:blue' ><b>Click Here</b></a>";
...
// format the date strings in the format dd-mm-yyyy
const todayStr = `${today.getDate().toString().padStart(2, '0')}-${(today.getMonth() + 1).toString().padStart(2, '0')}-${today.getFullYear().toString()}`;
const sevenDaysAgoStr = `${sevenDaysAgo.getDate().toString().padStart(2, '0')}-${(sevenDaysAgo.getMonth() + 1).toString().padStart(2, '0')}-${sevenDaysAgo.getFullYear().toString()}`;

// query the documents created after the specified date
const count = await DailyUpdate.countDocuments({ date: { $gte: sevenDaysAgoStr } });

if (count > 0) {
 user.user_inactive=false
} else {
  user.user_inactive=true
}
...
exports.updateProfile = async (req,res) => {
  ...
  try {
    const userCheckData= await User.findById(req.params._id)
console.log(userCheckData.email);
if(userCheckData.email==req.body.email){
  return res.status(200).send({msg:"email already registered",status:false})
}else{
...
exports.changePassword = async (req, res) => {
  ...
  const hasedPassword = await bcrypt.hash(req.body.new_password, salt);
  const user = await User.findOne({ _id: req.params._id });
  const valid_password = await bcrypt.compare(req.body.current_password, user.password);
...
exports.changePin = async (req, res) => {
  ...
  if (!validPin) return res.status(203).json({msg:"Incorrect Pin",status:false});
  try {
    if (req.body.token === req.body.confirm_token) {
      const updatedToken = await User.updateOne(
        { _id: req.params._id },
      ...
      return res.json({data:updatedToken,msg:"Pin changed successfully"});
    } else {
      return res.json({msg:"Token Not matched"});
    }
  } catch (error) {
...
exports.getAll = async(req,res) => {
  try {
    const nafs =  await User.find({fcm_token:{"$ne": null}})
    return res.status(200).json({data:nafs})
  } catch (error) {
    return res.status(400).json(error)
  }
...
exports.allNafs=async(req,res)=>{
  try {
    const nafs =  await Nafs.find()
    return res.status(200).json({data:nafs})
  ...
  const upload = multer({
    ...
    upload(req, res, function (err) {
      if (err) {
        ...
        const filename = req.files[0].originalname;
        return res.status(200).json({ data: `familyTree/${filename}`, msg: "Image saved successfully" });
      }
    });
  };
