...
const DailyMoodGraphModel=require("../models/DailyMoodGraphModel")
...
console.log("hereee")
if(req.body.moods.length>3){
    return  res.status(200).json({msg:"Moods more than 3"});
}
    ...
    const dailyMoodsGraph =  new DailyMoodGraphModel({
        moods:req.body.moods,
        rating:req.body.rating,
        user_id:req.body.user_id,
        work:req.body.work,
    ...
    if (Checkdata.length === 0) {
        try {
            ...
            const savemoodsgraph = await dailyMoodsGraph.save();
            res.status(200).json({data:savemoods,msg:"dailyMood updated successfully"});
            } catch (error) {
                res.status(400).send(error);
            }
    }
    else{
        console.log(Checkdata[0].rating,"here")
        if(Checkdata[0].rating.length >=4){
            return  res.status(200).json({msg:"Cannot enter more than 4 moods per day"});
        }
        const savemoodsgraph = await dailyMoodsGraph.save();
        ...
        try {
            ...
            var result = [];
                for (var i=0; i<7; i++) {
                    ...
                    const graphData = await  DailyMoodGraphModel.find({user_id:userId,date:formatDate(d)}).populate({path: 'moods', model: 'Mood'});

console.log(graphData)
                    console.log(graphData,i,"fff");
...
let day = d.getUTCDate();
                  let newObj={
                      ...
                      data:graphData
                        ...
                        //     newObj.rating=graphData[0].rating
...
exports.view = async (req,res) => {
    ...
    try {
      ...
      var decoded = jwt.verify(authorization, process.env.TOKEN_SECRET);
      var userId = decoded._id;
      const moodData = await  DailyMoodModel.find({ user_id:req.params.id,date:req.params.date}).populate('moods');
      if(moodData.length === 0){
  

        return res.status(200).json(({data:moodData ,msg:`No data for the date${eq.params.date}`}));
      }
      return res.status(200).json(({data:moodData,msg:"Mood data sent Successfully."}));
    } catch (error) {
       return res.status(400).json(error);
    }
        ...
        try {
            ...
            if (dailyMood.length == 0) {
               ...
               if (recentData.length == 0) {
                ...
                return res.json({moodList:moodList,msg:"dailyMood displayed successfully"});
               }
               return res.json({data:recentData,moodList:moodList,msg:"dailyMood displayed successfully"});
            } else{
                return res.json({data:dailyMood,moodList:moodList,msg:"dailyMood displayed successfully"});
